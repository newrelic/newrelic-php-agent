...
void nr_php_pgsql_save_datastore_instance(const zval* pgsql_conn,
  ...
  NRCTXGLOBAL(pgsql_last_conn) = key;
}
...
nr_datastore_instance_t* nr_php_pgsql_retrieve_datastore_instance(
  ...
  if (NULL == pgsql_conn) {
     ...
     */
    if (NRCTXGLOBAL(pgsql_last_conn)) {
      key = nr_strdup(NRCTXGLOBAL(pgsql_last_conn));
    } else {
...
void nr_php_pgsql_remove_datastore_instance(const zval* pgsql_conn TSRMLS_DC) {
   ...
   */
  if ((NULL == pgsql_conn) && (NRCTXGLOBAL(pgsql_last_conn))) {
    key = nr_strdup(NRCTXGLOBAL(pgsql_last_conn));
  } else {
    key = nr_php_datastore_make_key(pgsql_conn, "pgsql");
  }
  ...
  if (0 == nr_strcmp(key, NRCTXGLOBAL(pgsql_last_conn))) {
    nr_free(NRCTXGLOBAL(pgsql_last_conn));
  }
