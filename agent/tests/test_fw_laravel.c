...
static bool simulate_request_and_report_exception(const char* script) {
  ...
  tlib_pass_if_int_equal("framework detected", NRPRG_SHARED(current_framework),
                         NR_FW_LARAVEL);
...
static void test_exception_handler_hooks_on_inherited_methods_warm_worker(
   ...
   * leaves later requests silently uninstrumented.
   */
  recorded_request_1 = simulate_request_and_report_exception(
      "require '" PHP_SCRIPTS_DIR
      "/laravel/laravel_exception_handler_custom.php';");
  tlib_pass_if_true(
...
static void test_exception_handler_hooks_on_base_methods_warm_worker(TSRMLS_D) {
  bool recorded_request_1;
  bool recorded_request_2;
  ...
  recorded_request_1 = simulate_request_and_report_exception(
      "require '" PHP_SCRIPTS_DIR
      "/laravel/laravel_exception_handler_base.php';");
  tlib_pass_if_true(
      "The first request on a warm worker should record the exception",
      recorded_request_1, "recorded=%d", recorded_request_1);
