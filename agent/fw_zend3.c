 ...
 * this example from the Zend Skeleton App tutorial
 ...
 */
NR_PHP_WRAPPER(nr_zend3_name_the_wt) {
  zval* path = NULL;
  zval* this_var = NULL;
  ...
  NR_PHP_WRAPPER_REQUIRE_FRAMEWORK(NR_FW_ZEND3);
...
void nr_fw_zend3_enable(TSRMLS_D) {
  nr_txn_set_path("Zend3", NRPRG(txn), "unknown", NR_PATH_TYPE_ACTION,
                  NR_OK_TO_OVERWRITE);

  nr_php_wrap_user_function(
      NR_PSTR("Zend\\Router\\HTTP\\RouteMatch::setMatchedRouteName"),
      nr_zend3_name_the_wt TSRMLS_CC);
  nr_php_wrap_user_function(
