 ...
 *           starting a transaction or segment for a function that has just
 ...
 *           ending a transaction or segment for a function that has just ended.
 * This function is registered via the Observer API and will be called by the
 * zend engine every time a function ends.  The zend engine directly provides
 * the zend_execute_data and the return_value pointer, both of which have all
 * details that the agent needs to know about the function. This and
