...
/*DESCRIPTION
Test removing a curl handle while curl multi exec is in flight if fibers are involved.
*/
  ...
  [
    {
      ...
      "traceId": "??",
      "transactionId": "??",
      "name": "Custom\/test_curl_multi_exec_remove_handles",
      "timestamp": "??",
      "duration": "??",
      "priority": "??",
      "sampled": true,
...
[
  ...
  [
    ...
    [{"name":"Supportability/TraceContext/Create/Success"},           [6, "??", "??", "??", "??", "??"]],
    [{"name":"Supportability/DistributedTrace/CreatePayload/Success"},[6, "??", "??", "??", "??", "??"]],
    [{"name":"External/all"},                                         [3, "??", "??", "??", "??", "??"]],
...
{
    env_var_for_expects("GUID_TEST_REMOVE", newrelic_get_linking_metadata()['span.id'] ?? '');
...
$fiber_a = new Fiber('a');
$fiber_remove = new Fiber('test_curl_multi_exec_remove_handles');

$fiber_remove->start();
$fiber_a->start();
$result = $fiber_remove->resume();
do {
  $result = $fiber_remove->resume();
} while ($result > 0);
$fiber_a->resume();
