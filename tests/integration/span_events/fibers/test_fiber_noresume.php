...
Should properly close spans when caller exits even if fiber->resume doesn't happen, but no more execution will occur in the fiber.

Here is what happens to the suspended fiber and its state:

Pending finally blocks: Any finally blocks within the fiber's current execution stack will be executed, 
allowing for cleanup (e.g., closing file handles, database connections).

Destructors: The __destruct() methods of any objects held in the fiber’s scope will be called.

No further execution: The code within the fiber will not complete, and it will not return to t
he Fiber::suspend() call to resume its normal flow.

Memory release: The fiber's stack and associated memory are freed once the Fiber object has no 
remaining references and is collected.
