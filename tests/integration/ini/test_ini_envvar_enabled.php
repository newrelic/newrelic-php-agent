...
/*DESCRIPTION
Verify all ini values can be set via env var equivlents.

NOTE: Some env values are ignored - if set to value other than what 
      the integration_runner expects then no harvest is sent and test fails

        NEW_RELIC_APPNAME
        NEW_RELIC_DAEMON_PORT
        NEW_RELIC_ENABLED
        NEW_RELIC_LICENSE

      Since some of these are likely to be used by the customer there will need to
      be complementary tests (cross platform?) that use these env values in a 
      actual live environment and confirm the agent is able to send data

NOTE: Since the newreloc.logfile is changed by this test the "php_agent.log" file
      in the directory the test is run from will be truncated when the change
      in made when the agent in initializing.  If the full agent log is required
      then simply disable the NEW_RELIC_LOGFILE environment variable below
*/
...
Supportability/Logging/Forwarding/PHP/disabled
Supportability/Logging/Metrics/PHP/disabled
S
*/
...
foreach ($names as $ini_name => $env_name) {
    ...
    if ($local_val == $global_val) {
        $err_str .= "     - $ini_name not different from default (uses $env_name):  current = '" . $local_val . "' default = '" . $global_val . "', env = " . $env_val . "\n";
    } else {
        //keep for debugging
        //$err_str .= "PASS   $ini_name     different from default (uses $env_name):  current = '" . $local_val . "' default = '" . $global_val . "', env = " . getenv($env_name) . "\n";
    }
}

if (0 != strlen($err_str)) {
    echo "The following errors were observed:\n\n";
    echo $err_str . "\n";
}